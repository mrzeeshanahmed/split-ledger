...
export const WebhookDetailPage: React.FC = () => {
    ...
    const handleTestEvent = async () => {
        ...
        try {
            setIsTesting(true);
            const res = await testWebhook(webhook.id, 'webhook.test', { message: 'Test ping from Split-Ledger Dashboard' });
            addToast({
            ...
            <div className="p-6 max-w-7xl mx-auto space-y-6">
                ...
                <div className="flex justify-between items-start">
                    <div className="space-y-1">
                        <h1 className="text-2xl font-semibold text-text-primary break-all">{webhook.url}</h1>
                        ...
                        {webhook.description && (
                            <p className="text-sm text-text-muted mt-2">{webhook.description}</p>
                        )}
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <StatCard
                        ...
                        title="Successful"
                        value={stats.success}
                        change={stats.total > 0 ? { value: Math.round((stats.success / stats.total) * 100), type: 'increase' } : undefined}
                    />
                    <StatCard
                        label="Failed"
                        title="Failed"
                ...
                {/* Deliveries Logs Table Component */}
                <div>
                    <h2 className="text-lg font-medium text-text-primary mb-4">Delivery History</h2>
                    <DeliveryLogsTable webhookId={webhook.id} />
                </div>
